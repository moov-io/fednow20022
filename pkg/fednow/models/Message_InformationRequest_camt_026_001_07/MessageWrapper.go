...
type MessageInformationRequestWrapper struct{}

func (w *MessageInformationRequestWrapper) CreateDocument(modelJson []byte) ([]byte, error) {
	...
	dataDoc, err := common.DocumentWith(model.InformationRequest, BodyPathMap, BodyDocumentFactory)
	if err != nil {
...
func (w *MessageInformationRequestWrapper) ValidateDocument(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
...
func (w *MessageInformationRequestWrapper) CheckRequireField(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
		return fmt.Errorf("failed to unmarshal JSON: %w", err)
	}
	...
	dataJson, err := json.Marshal(model.InformationRequest)
	if err != nil {
...
func (w *MessageInformationRequestWrapper) GetDataModel(xmlData []byte) (modelJson []byte, err error) {
	var doc GeneratedMsgType
	...
	bodyModelModel, err := common.MessageModelWith(doc.InformationRequest, BodyDataFactory, BodyPathMap)
	if err != nil {
		return nil, err
	}
...
func (w *MessageInformationRequestWrapper) GetHelp() ([]byte, error) {
	helper := BuildHelper()
