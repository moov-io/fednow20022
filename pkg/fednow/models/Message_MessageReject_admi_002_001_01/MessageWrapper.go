...
type MessageMessageRejectWrapper struct{}

func (w *MessageMessageRejectWrapper) CreateDocument(modelJson []byte) ([]byte, error) {
	...
	dataDoc, err := common.DocumentWith(model.MessageReject, BodyPathMap, BodyDocumentFactory)
	if err != nil {
...
func (w *MessageMessageRejectWrapper) ValidateDocument(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
...
func (w *MessageMessageRejectWrapper) CheckRequireField(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
		return fmt.Errorf("failed to unmarshal JSON: %w", err)
	}
	...
	dataJson, err := json.Marshal(model.MessageReject)
	if err != nil {
...
func (w *MessageMessageRejectWrapper) GetDataModel(xmlData []byte) (modelJson []byte, err error) {
	var doc GeneratedMsgType
	...
	bodyModelModel, err := common.MessageModelWith(doc.MessageReject, BodyDataFactory, BodyPathMap)
	if err != nil {
		return nil, err
	}
...
func (w *MessageMessageRejectWrapper) GetHelp() ([]byte, error) {
	helper := BuildHelper()
