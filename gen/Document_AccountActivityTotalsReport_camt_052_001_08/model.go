// Code generated by GoComply XSD2Go for Moov; DO NOT EDIT.
// Models for urn:fednow
package Document_AccountActivityTotalsReport_camt_052_001_08

import (
	"encoding/xml"

	"github.com/moov-io/fednow20022/gen/AccountActivityTotalsReport_camt_052_001_08"
	"github.com/moov-io/fednow20022/gen/head_001_001_02"
)

// XSD Elements

type Message struct {
	XMLName xml.Name   `xml:"Message"`
	Xmlns   []xml.Attr `xml:",attr"`

	AppHdr head_001_001_02.BusinessApplicationHeaderV02 `xml:"urn:fednow AppHdr"`

	AccountActivityTotalsReport *AccountActivityTotalsReport_camt_052_001_08.Document `xml:"urn:fednow AccountActivityTotalsReport,omitempty"`
}

var NamespacePrefixMap = map[string]string{
	"urn:iso:std:iso:20022:tech:xsd:head.001.001.01": "head",
	"urn:iso:std:iso:20022:tech:xsd:camt.052.001.08": "aatr",
}

func (v *Message) Body() interface{} {
	if v.AccountActivityTotalsReport != nil {
		return v.AccountActivityTotalsReport
	}
	return nil
}

// XSD ComplexType declarations
