...
type Message_AccountDebitCreditNotificationWrapper struct{}

func (w *Message_AccountDebitCreditNotificationWrapper) CreateDocument(modelJson []byte) ([]byte, error) {
	...
	dataDoc, err := common.DocumentWith(model.AccountDebitCreditNotification, BodyPathMap, BodyDocumentFactory)
	if err != nil {
	...
	doc := GeneratedMsgType{
		Xmlns:                          XMLAttributes,
		AppHdr:                         hdr,
		AccountDebitCreditNotification: data,
	}
...
func (w *Message_AccountDebitCreditNotificationWrapper) ValidateDocument(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
...
func (w *Message_AccountDebitCreditNotificationWrapper) CheckRequireField(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
		return fmt.Errorf("failed to unmarshal JSON: %w", err)
	}
	...
	dataJson, err := json.Marshal(model.AccountDebitCreditNotification)
	if err != nil {
...
func (w *Message_AccountDebitCreditNotificationWrapper) GetDataModel(xmlData []byte) (modelJson []byte, err error) {
	var doc GeneratedMsgType
	...
	bodyModelModel, err := common.MessageModelWith(doc.AccountDebitCreditNotification, BodyDataFactory, BodyPathMap)
	if err != nil {
		return nil, err
	}
...
func (w *Message_AccountDebitCreditNotificationWrapper) GetHelp() ([]byte, error) {
	helper := BuildHelper()
