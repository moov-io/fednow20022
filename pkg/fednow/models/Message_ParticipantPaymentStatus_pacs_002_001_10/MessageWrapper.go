...
type MessageParticipantPaymentStatusWrapper struct{}

func (w *MessageParticipantPaymentStatusWrapper) CreateDocument(modelJson []byte) ([]byte, error) {
	...
	dataDoc, err := common.DocumentWith(model.ParticipantPaymentStatus, BodyPathMap, BodyDocumentFactory)
	if err != nil {
...
func (w *MessageParticipantPaymentStatusWrapper) ValidateDocument(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
...
func (w *MessageParticipantPaymentStatusWrapper) CheckRequireField(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
		return fmt.Errorf("failed to unmarshal JSON: %w", err)
	}
	...
	dataJson, err := json.Marshal(model.ParticipantPaymentStatus)
	if err != nil {
...
func (w *MessageParticipantPaymentStatusWrapper) GetDataModel(xmlData []byte) (modelJson []byte, err error) {
	var doc GeneratedMsgType
	...
	bodyModelModel, err := common.MessageModelWith(doc.ParticipantPaymentStatus, BodyDataFactory, BodyPathMap)
	if err != nil {
		return nil, err
	}
...
func (w *MessageParticipantPaymentStatusWrapper) GetHelp() ([]byte, error) {
	helper := BuildHelper()
