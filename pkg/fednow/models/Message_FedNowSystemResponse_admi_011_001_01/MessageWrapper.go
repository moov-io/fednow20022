...
type MessageFedNowSystemResponseWrapper struct{}

func (w *MessageFedNowSystemResponseWrapper) CreateDocument(modelJson []byte) ([]byte, error) {
	...
	dataDoc, err := common.DocumentWith(model.FedNowSystemResponse, BodyPathMap, BodyDocumentFactory)
	if err != nil {
...
func (w *MessageFedNowSystemResponseWrapper) ValidateDocument(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
...
func (w *MessageFedNowSystemResponseWrapper) CheckRequireField(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
		return fmt.Errorf("failed to unmarshal JSON: %w", err)
	}
	...
	dataJson, err := json.Marshal(model.FedNowSystemResponse)
	if err != nil {
...
func (w *MessageFedNowSystemResponseWrapper) GetDataModel(xmlData []byte) (modelJson []byte, err error) {
	var doc GeneratedMsgType
	...
	bodyModelModel, err := common.MessageModelWith(doc.FedNowSystemResponse, BodyDataFactory, BodyPathMap)
	if err != nil {
		return nil, err
	}
...
func (w *MessageFedNowSystemResponseWrapper) GetHelp() ([]byte, error) {
	helper := BuildHelper()
