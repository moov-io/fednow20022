...
type MessageCustomerCreditTransferWrapper struct{}

func (w *MessageCustomerCreditTransferWrapper) CreateDocument(modelJson []byte) ([]byte, error) {
	...
	dataDoc, err := common.DocumentWith(model.CustomerCreditTransfer, BodyPathMap, BodyDocumentFactory)
	if err != nil {
	...
	if !ok {
		return nil, fmt.Errorf("data document is not of type *CustomerCreditTransfer_camt_052_001_08.Document")
	}

	doc := GeneratedMsgType{
		Xmlns:                  XMLAttributes,
		AppHdr:                 hdr,
		CustomerCreditTransfer: data,
	}
...
func (w *MessageCustomerCreditTransferWrapper) ValidateDocument(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
...
func (w *MessageCustomerCreditTransferWrapper) CheckRequireField(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
		return fmt.Errorf("failed to unmarshal JSON: %w", err)
	}
	...
	dataJson, err := json.Marshal(model.CustomerCreditTransfer)
	if err != nil {
...
func (w *MessageCustomerCreditTransferWrapper) GetDataModel(xmlData []byte) (modelJson []byte, err error) {
	var doc GeneratedMsgType
	...
	bodyModelModel, err := common.MessageModelWith(doc.CustomerCreditTransfer, BodyDataFactory, BodyPathMap)
	if err != nil {
		return nil, err
	}
...
func (w *MessageCustomerCreditTransferWrapper) GetHelp() ([]byte, error) {
	helper := BuildHelper()
