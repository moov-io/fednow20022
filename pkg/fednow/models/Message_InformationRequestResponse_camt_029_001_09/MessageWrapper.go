...
type MessageInformationRequestResponseWrapper struct{}

func (w *MessageInformationRequestResponseWrapper) CreateDocument(modelJson []byte) ([]byte, error) {
	...
	dataDoc, err := common.DocumentWith(model.InformationRequestResponse, BodyPathMap, BodyDocumentFactory)
	if err != nil {
	...
	doc := GeneratedMsgType{
		Xmlns:                      XMLAttributes,
		AppHdr:                     hdr,
		InformationRequestResponse: data,
	}
...
func (w *MessageInformationRequestResponseWrapper) ValidateDocument(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
...
func (w *MessageInformationRequestResponseWrapper) CheckRequireField(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
		return fmt.Errorf("failed to unmarshal JSON: %w", err)
	}
	...
	dataJson, err := json.Marshal(model.InformationRequestResponse)
	if err != nil {
...
func (w *MessageInformationRequestResponseWrapper) GetDataModel(xmlData []byte) (modelJson []byte, err error) {
	var doc GeneratedMsgType
	...
	bodyModelModel, err := common.MessageModelWith(doc.InformationRequestResponse, BodyDataFactory, BodyPathMap)
	if err != nil {
		return nil, err
	}
...
func (w *MessageInformationRequestResponseWrapper) GetHelp() ([]byte, error) {
	helper := BuildHelper()
