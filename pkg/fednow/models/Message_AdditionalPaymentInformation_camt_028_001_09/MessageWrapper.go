...
type MessageAdditionalPaymentInformationWrapper struct{}

func (w *MessageAdditionalPaymentInformationWrapper) CreateDocument(modelJson []byte) ([]byte, error) {
	...
	dataDoc, err := common.DocumentWith(model.AdditionalPaymentInformation, BodyPathMap, BodyDocumentFactory)
	if err != nil {
	...
	if !ok {
		return nil, fmt.Errorf("data document is not of type *AdditionalPaymentInformation_camt_052_001_08.Document")
	}
...
func (w *MessageAdditionalPaymentInformationWrapper) ValidateDocument(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
...
func (w *MessageAdditionalPaymentInformationWrapper) CheckRequireField(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
		return fmt.Errorf("failed to unmarshal JSON: %w", err)
	}
	...
	dataJson, err := json.Marshal(model.AdditionalPaymentInformation)
	if err != nil {
...
func (w *MessageAdditionalPaymentInformationWrapper) GetDataModel(xmlData []byte) (modelJson []byte, err error) {
	var doc GeneratedMsgType
	...
	bodyModelModel, err := common.MessageModelWith(doc.AdditionalPaymentInformation, BodyDataFactory, BodyPathMap)
	if err != nil {
		return nil, err
	}
...
func (w *MessageAdditionalPaymentInformationWrapper) GetHelp() ([]byte, error) {
	helper := BuildHelper()
