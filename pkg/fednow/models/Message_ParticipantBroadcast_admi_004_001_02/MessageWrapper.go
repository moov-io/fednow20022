...
type MessageParticipantBroadcastWrapper struct{}

func (w *MessageParticipantBroadcastWrapper) CreateDocument(modelJson []byte) ([]byte, error) {
	...
	dataDoc, err := common.DocumentWith(model.ParticipantBroadcast, BodyPathMap, BodyDocumentFactory)
	if err != nil {
...
func (w *MessageParticipantBroadcastWrapper) ValidateDocument(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
...
func (w *MessageParticipantBroadcastWrapper) CheckRequireField(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
		return fmt.Errorf("failed to unmarshal JSON: %w", err)
	}
	...
	dataJson, err := json.Marshal(model.ParticipantBroadcast)
	if err != nil {
...
func (w *MessageParticipantBroadcastWrapper) GetDataModel(xmlData []byte) (modelJson []byte, err error) {
	var doc GeneratedMsgType
	...
	bodyModelModel, err := common.MessageModelWith(doc.ParticipantBroadcast, BodyDataFactory, BodyPathMap)
	if err != nil {
		return nil, err
	}
...
func (w *MessageParticipantBroadcastWrapper) GetHelp() ([]byte, error) {
	helper := BuildHelper()
