...
type MessageFedNowPaymentStatusWrapper struct{}

func (w *MessageFedNowPaymentStatusWrapper) CreateDocument(modelJson []byte) ([]byte, error) {
	...
	dataDoc, err := common.DocumentWith(model.FedNowPaymentStatus, BodyPathMap, BodyDocumentFactory)
	if err != nil {
...
func (w *MessageFedNowPaymentStatusWrapper) ValidateDocument(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
...
func (w *MessageFedNowPaymentStatusWrapper) CheckRequireField(modelJson []byte) error {
	var model MessageModel
	if err := json.Unmarshal(modelJson, &model); err != nil {
		return fmt.Errorf("failed to unmarshal JSON: %w", err)
	}
	...
	dataJson, err := json.Marshal(model.FedNowPaymentStatus)
	if err != nil {
...
func (w *MessageFedNowPaymentStatusWrapper) GetDataModel(xmlData []byte) (modelJson []byte, err error) {
	var doc GeneratedMsgType
	...
	bodyModelModel, err := common.MessageModelWith(doc.FedNowPaymentStatus, BodyDataFactory, BodyPathMap)
	if err != nil {
		return nil, err
	}
...
func (w *MessageFedNowPaymentStatusWrapper) GetHelp() ([]byte, error) {
	helper := BuildHelper()
